-- Vytal Tek entitlement store (Phase G)
-- Apply only to a dedicated Vytal project — not shared product databases.
...
-- Users read only their own entitlement row. Writes are service-role only.
